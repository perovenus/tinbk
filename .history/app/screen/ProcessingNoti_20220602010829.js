...
const ProcessingNoti = ({notification}) => {
  ...
  const gotoProductInfo = i => {
    ...
    firestore()
      ...
      .then(doc => {
        bookdata = doc.data();
        console.log('Đây là bookdata', bookdata.orderList.includes(user.uid));
  ...
  const gotoPartnerInfoScreen = notification => {
    //get partner info from firebase
    let partnerInfo = {};
    firestore()
      .collection('Users')
      .doc(notification.partner)
      .get()
      .then(doc => {
        partnerInfo = doc.data();
        console.log('Đây là partnerInfo', partnerInfo);
        Actions.PartnerInfo(partnerInfo);
      });
  };
